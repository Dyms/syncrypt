...
rides entirely on the LIST-based protocol, which is exactly why this provider
exists: it proves the abstraction with a second, protocol-different backend.
...
object. Injectable transport: Obsidian clients pass the same
`requestUrl()`-backed transport as for S3.

Testing: the shared conformance suite and an encrypted two-device
